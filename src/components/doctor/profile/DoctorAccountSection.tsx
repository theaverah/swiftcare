...
export function DoctorAccountSection({ email, onDirtyChange, onClose }: Props) {
  ...
  function handleChange(setter: (v: string) => void) {
    return (e: React.ChangeEvent<HTMLInputElement>) => { setter(e.target.value); onDirtyChange(true); };
  }
  ...
  async function handleDelete() {
    setDeleting(true);
    try {
      const res = await fetch("/api/doctor/account/delete", { method: "DELETE" });
    ...
    >
      ...
      >
        ...
        <p className="text-[16px] text-text-sub text-center mb-7">
          This will permanently delete your doctor profile, all your consultations, and patient records linked to your account. This cannot be undone.
        </p>
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <div className="h-11 px-3 rounded-lg border border-elements bg-bg-sub flex items-center">
          <p className="text-[16px] text-text-sub">{email}</p>
        </div>
      ...
      <div className="flex items-center justify-between">
        <div className="flex flex-col gap-0.5">
          ...
          <p className="text-[16px] text-text-sub">Permanently delete your doctor profile and all associated data.</p>
        </div>
        <button
          type="button"
